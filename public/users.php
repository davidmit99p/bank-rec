...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'add') {
            ...
            if (!$ok) throw new RuntimeException($msg);
            log_event('reset a password', user_name($id));
...
<div class="panel">
  <div class="scroll">
    <table>
      <thead><tr><th>Name</th><th>Username</th><th>Role</th><th>Last signed in</th><th></th></tr></thead>
      <tbody>
          ...
          <td>
            <div style="display:flex;gap:.3rem;flex-wrap:wrap;align-items:center">
              <form method="post" style="display:flex;gap:.3rem;align-items:center">
                ...
                <input type="hidden" name="to" value="<?= $u['role'] === 'admin' ? 'user' : 'admin' ?>">
                <button class="btn ghost small" type="submit">Make <?= $u['role'] === 'admin' ? 'a user' : 'an administrator' ?></button>
              </form>
              <form method="post">
